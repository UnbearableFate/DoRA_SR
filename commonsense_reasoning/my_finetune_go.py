...
from trainer.svd_ref_trainer import get_warmup_restart_then_final_decay_scheduler_ratio, restart_init_train ,DistributedSvdRefactorTrainer
import wandb
...
def train(
    ...
    if adjust_lora_alpha == 1:
        wandb_run_name += f"&alpha{min_alpha_ratio}~{max_alpha_ratio}@start"
    ...
    common_args = {
        ...
        "args": transformers.TrainingArguments(
            ...
            max_steps= 5294,  # set max_steps to a large number, actual steps controlled by sr_init_steps and trainer
    ...
    scheduler = get_warmup_restart_then_final_decay_scheduler_ratio(
        optimizer=optimizer,
        num_training_steps=5294,
        repeat_n=2,
        repeat_warmup_ratio=0.03,   # 2% of total steps
        repeat_decay_ratio=0.03,    # 8% of total steps
        repeat_end_lr_rate=0.97,     # decay to 30% of max lr
        final_warmup_ratio=0.02,
        min_lr_rate=0.01,
